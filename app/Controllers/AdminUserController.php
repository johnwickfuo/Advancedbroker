...
final class AdminUserController extends Controller {
    ...
    public function endPopup(Request $request): Response {
        $user=$this->target($request);
        if(!$user)return Response::redirect(route('admin.users.index'));
        $notificationId=(int)$request->route('notification');
        if(app('notifications')->endAdminPopup((int)$user['id'],$notificationId)){
            app('audit')->record((int)$_SESSION['user_id'],'user.popup_ended','user',(int)$user['id'],[],['notification_id'=>$notificationId],null,$request);
            $this->flash('success','Popup notification ended. It will not appear again.');
        }else{
            $this->flash('error','That popup is already complete or could not be found.');
        }
        return Response::redirect(route('admin.users.show',['user'=>$user['id']]));
    }
