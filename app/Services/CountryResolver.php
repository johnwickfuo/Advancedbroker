...
final class CountryResolver {
    ...
    public function resolve(Request $request, ?array $account = null): CountryContext {
        ...
        if (($_SESSION['user_role'] ?? '') === 'super_admin' && !empty($_SESSION['admin_country_preview']['country_id'])) {
            ...
            if($preview)return new CountryContext($preview,(string)($_SESSION['admin_country_preview']['language']?:$this->countries->defaultLanguage($preview)),$geo,'admin_preview');
        }
        if ($account && !empty($account['id'])) {
            foreach (['assigned_country_id' => 'admin_assignment', 'country_id' => 'stored_account_country'] as $field => $source) {
                ...
                if($id && ($country=$this->countries->byId($id))) return new CountryContext($country,$this->countries->defaultLanguage($country),$geo,$source);
            }
        ...
        if ($this->debug && ($preview=(string)$request->input('preview_country',$_SESSION['preview_country']??''))) {
            ...
            if($country){$_SESSION['preview_country']=$preview;return new CountryContext($country,$this->countries->defaultLanguage($country),$geo,'development_preview');}
        }
