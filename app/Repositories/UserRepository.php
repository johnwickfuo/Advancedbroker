...
final class UserRepository {
    ...
    public function update(int $id,array $data): void {
        if(!$this->db) throw new \RuntimeException('Database unavailable.');
        $allowed=['username','first_name','last_name','phone','email','password_hash','country_id','assigned_country_id','country_assignment_source','original_country_id','preferred_language_id','detected_country_code','account_status','role','account_restriction_reason','email_verified_at','last_login_at','two_factor_secret_encrypted','two_factor_enabled_at'];
        $sets=[];$values=[];
    ...
    public function search(string $query='',int $limit=100): array {
        ...
        $like='%'.$query.'%';
        return $this->db->select('SELECT u.*,l.code preferred_language_code,c.name country_name FROM users u LEFT JOIN languages l ON l.id=u.preferred_language_id LEFT JOIN countries c ON c.id=COALESCE(u.assigned_country_id,u.country_id) WHERE u.email LIKE ? OR u.username LIKE ? OR u.first_name LIKE ? OR u.last_name LIKE ? OR u.uuid=? ORDER BY u.created_at DESC LIMIT '.$limit,[$like,$like,$like,$like,$query]);
    }
