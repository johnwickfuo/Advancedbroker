...
use App\Services\{AccountAccessService,AiTradingService,AuditLogService,AuthService,BrandingService,BrowserMarketLockService,CompanyService,ContactService,ContentService,CountryAccessService,CountryAdminService,CountryResolver,CountryService,DepositService,DynamicFormService,EmailVerificationService,FaqService,CountryMediaService,GeoLocationService,OfflinePageTranslationService,HomepageExperienceService,InvestmentService,KycService,LanguageService,LegalService,LicenseService,NotificationService,OfferingCalculator,OfferingValidator,PasswordResetService,QrCodeService,SecurityEventService,SessionService,SettingsService,ThemeService,Translator,TwoFactorService,UploadService,UserPreferenceService,WalletService,WithdrawalService};
...
final class Application {
    public static function boot(string $basePath): Router {
        ...
        $container->set('countries',$countries); $container->set('homepage_experience',new HomepageExperienceService()); $container->set('country_media',new CountryMediaService()); $container->set('market_lock',new BrowserMarketLockService((string)$config['app']['key'])); $container->set('geo',new GeoLocationService($config['geoip'],$container->get('cache'))); $container->set('country_resolver',new CountryResolver($countries,$container->get('geo'),(bool)$config['app']['debug'])); $container->set('languages',new LanguageService($countries)); $container->set('translator',new Translator($basePath.'/resources/lang',$config['localization']['available_languages'])); $container->set('theme',new ThemeService()); $container->set('country_access',new CountryAccessService()); $container->set('user_preferences',new UserPreferenceService($database));
